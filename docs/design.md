...
- `PortfolioHistoryService` 只沉淀账户级时间序列，不参与当前快照构建。
- `ScannerService` 不写入快照，不修改 watchlist。
- `AlertRuleEngine` 只判断规则，不直接发 Telegram / 飞书。
- `Telegram / Feishu adapter` 只发送消息，不决定是否提醒。
...
- `NotificationService`：通知发送和规则统计
- `EventService`：全局事件时间线、事件保留清理
...
- `scanner`：策略线索候选
- `watchlist`：关注标的基础信息
- `preferences`：IBKR / Telegram / 飞书 / 快照刷新设置

领域层原则：

- 不直接依赖数据库 session
- 不直接依赖 IBKR、Telegram、飞书、FastAPI
- 尽量保持同输入同输出
...
- `Alerts`：提醒规则管理和发送统计
- `Portfolio`：账户和持仓
- `Settings`：IBKR、Telegram、飞书、快照刷新配置
...
- 前端手动刷新有超时提示，不会一直转圈。
- 每次快照刷新都会写入 `events`，来源标记为 `initial / manual / automatic`。
- 事件默认保留 `90 天`，最多 `10000 条`，写入新事件后自动清理。
...
- 高低点来自 IBKR 历史 K 线；自动化测试会使用测试专用 IBKR 客户端生成稳定数据。
...
- 不复制数据库也能运行。
- 不复制数据库会丢失本地 watchlist、提醒规则、IBKR/Telegram/飞书页面配置、事件时间线。
