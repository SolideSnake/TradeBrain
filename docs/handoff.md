...
2. 完善策略计划模型，把买入区间、卖出区间、止损止盈从 watchlist 外提出来
3. 把 `/api/scanner` 的策略线索扩展成可配置扫描条件
4. 根据真实使用反馈决定是否加图表、标的详情页或 WebSocket
5. 再评估任务中心是否恢复
...
8. 如果要做 live 联调，再确认 TWS / IB Gateway 与 `.env` 配置
...
TradeBrain 第一版 MVP 已完成，当前主链路已经演进为 “IBKR/Mock -> SnapshotBuilder -> SnapshotPipeline -> 指标/状态/提醒 -> 快照缓存 -> Web 看板”，并新增 “CanonicalSnapshot -> StrategyEvaluator -> ScoringService -> ScannerService -> /api/scanner -> 追踪页策略线索”。下一步重点是把买卖计划从 watchlist 外提出来，进入独立策略计划模型。
