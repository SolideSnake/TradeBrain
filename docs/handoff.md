...
1. 在真实/模拟 TWS 下继续验证行情、PEG、账户字段稳定性
2. 观察 `区间位置` UI 在真实数据下是否过密，必要时拆出标的详情页
3. 完善策略计划模型，把买入区间、卖出区间、止损止盈从 watchlist 外提出来
4. 把 `/api/scanner` 的策略线索扩展成可配置扫描条件
5. 根据真实使用反馈决定是否加图表、标的详情页或 WebSocket
6. 再评估任务中心是否恢复
...
- 换电脑如果不复制数据库，项目仍能运行，只是本地 watchlist、设置、提醒历史需要重新配置。
...
TradeBrain 第一版 MVP 已完成，当前主链路已经演进为 “IBKR/Mock -> SnapshotBuilder -> SnapshotPipeline -> 指标/状态/提醒 -> 快照缓存 -> Web 看板”，并新增 “CanonicalSnapshot -> StrategyEvaluator -> ScoringService -> ScannerService -> /api/scanner -> 追踪页策略线索”。追踪页已具备 52W/90D 区间位置展示和刷新防卡住保护。下一步重点是把买卖计划从 watchlist 外提出来，进入独立策略计划模型。
