...
如果下一条线程要继续开发，当前最推荐的是单开一个“前端优化线程”。
...
- 不改状态引擎规则
- 不改安装和启动脚本
- 如果前端问题必须改后端，先记录建议，不在该线程直接改

建议优先事项：

1. 优化 `Overview` 页的信息层级
2. 优化 `Monitor` 页的筛选、排序、状态标签和表格可读性
3. 优化 `Alerts` 页的历史记录和错误展示
4. 优化 `Portfolio` 页的持仓表与盈亏展示
5. 优化 `Settings` 页的保存反馈和测试发送反馈
...
6. 确认前端页面可打开：
   ...
   - `Portfolio`
   - `Settings`
7. 如果要做 live 联调，再确认 TWS / IB Gateway 与 `.env` 配置
